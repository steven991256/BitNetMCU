...
def load_model(model_name, params):
    ...
    try:
        ...
        raise ValueError(
            f"Model {model_name} not found in models.py"
        )
...
def export_to_hfile(
    ...
    if not quantized_model.quantized_model:
        raise ValueError(
            "quantized_model is empty or None"
        )
    ...
    with open(filename, "w") as f:
        ...
        f.write(
            f"// Quantized model exported from {runname}.pth\n"
        )
        ...
        f.write(
            f"#define NUM_LAYERS "
            f"{len(quantized_model.quantized_model)}\n"
        )
        ...
        for layer_info in quantized_model.quantized_model:
            ...
            if layer_info["layer_type"] == "BitLinear":
                ...
                incoming_weights = layer_info[
                    "incoming_weights"
                ]

                outgoing_weights = layer_info[
                    "outgoing_weights"
                ]
                ...
                if (bpw * incoming_weights % 32) != 0:
                    ...
                    raise ValueError(
                        f"Size mismatch: incoming weights "
                        f"must pack to 32-bit boundary. "
                        f"Layer={layer}, "
                ...
                if quantization_type in (
                    ...
                    if n_inputs % 10 != 0:
                        ...
                        print(
                            f"WARNING: Ternary layer "
                            f"{layer} has {n_inputs} inputs, "
                    ...
                    f.write(
                        f"#define "
                        f"{layer}_bitperweight "
                        f"{QuantID}\n"
                    )

                    f.write(
                        f"#define "
                        f"{layer}_incoming_weights "
                        f"{n_inputs}\n"
                    )

                    f.write(
                        f"#define "
                        f"{layer}_outgoing_weights "
                        f"{outgoing_weights}\n"
                    )
            ...
            # =================================================
                ...
                incoming_x = layer_info[
                    "incoming_x"
                ]

                incoming_y = layer_info[
                    "incoming_y"
                ]

                outgoing_x = layer_info[
                    "outgoing_x"
                ]

                outgoing_y = layer_info[
                    "outgoing_y"
                ]
